...
public class Main {
	...
	static class position implements Comparable<position> {
		...
		public position(int row, int col, int crush) {
			super();
			this.row = row;
			this.col = col;
			this.crush = crush;
			...
			for (int i = 0; i < 4; i++) {
				...
				if (map[newR][newC] == 0) {
					if (!visited[newR][newC]) {
						...
						queue.add(new position(newR, newC, pos.crush));
					}
				} else if (map[newR][newC] == 1) {
					if (!visited[newR][newC]) {
						visited[newR][newC] = true;
						queue.add(new position(newR, newC, pos.crush + 1));
					}
				}
			}
		}
